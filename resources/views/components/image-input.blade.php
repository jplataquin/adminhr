...
<div id="image_upload_card_{{$name}}" disabled="{{$disabled}}"  {{ $attributes->merge(['class' => 'grid sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-4 p-4']) }}>
    ...
    <div id="image_upload_controls_{{$name}}">
        <div>
            ...
            <input type="file" id="input_{{$name}}" accept="image/*;capture=camera" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"/>
        </div>
